...
import backend.numpy_backend as numpy
import copy
import os
import time
...
# dataset iterators
import fuel.flickr8k as flickr8k
import fuel.flickr30k as flickr30k
import fuel.coco as coco
...
def param_init_fflayer(options, params, prefix='ff', nin=None, nout=None,**kwargs):
    if nin is None:
...
def param_init_lstm_cond(options, params, prefix='lstm_cond', nin=None, dim=None, dimctx=None):
    ...
    # context to LSTM
    Wc2h = norm_weight(dimctx,dim*4)
    params[_p(prefix,'Wc2h')] = Wc2h

    # attention: context -> hidden
    W_c2pc = norm_weight(dimctx, ortho=False)
    params[_p(prefix,'W_c2pc')] = W_c2pc

    # attention: LSTM -> hidden
    W_h2pc = norm_weight(dim,dimctx)
    params[_p(prefix,'W_h2pc')] = W_h2pc

    # attention: hidden bias
    b_c2pc = numpy.zeros((dimctx,)).astype('float32')
    params[_p(prefix,'b_c2pc')] = b_c2pc

    # optional "deep" attention
    if options['n_layers_att'] > 1:
        for lidx in xrange(1, options['n_layers_att']):
            params[_p(prefix,'W_c2pc_%d'%lidx)] = ortho_weight(dimctx)
            params[_p(prefix,'b_c2pc_%d'%lidx)] = numpy.zeros((dimctx,)).astype('float32')
...
def lstm_cond_layer(tparams, state_below, options, prefix='lstm',
    ...
    # projected context
    pctx_ = tensor.dot(context, tparams[_p(prefix,'W_c2pc')]) + tparams[_p(prefix, 'b_c2pc')]
    if options['n_layers_att'] > 1:
        for lidx in xrange(1, options['n_layers_att']):
            pctx_ = tensor.dot(pctx_, tparams[_p(prefix,'W_c2pc_%d'%lidx)])+tparams[_p(prefix, 'b_c2pc_%d'%lidx)]
    ...
    def _step(m_, x_, h_, c_, a_, as_, ct_, pctx_, dp_=None, dp_att_=None):
        ...
        pstate_ = tensor.dot(h_, tparams[_p(prefix,'W_h2pc')])
        ...
        preact = tensor.dot(h_, tparams[_p(prefix, 'U')])
        preact += x_
        preact += tensor.dot(ctx_, tparams[_p(prefix, 'Wc2h')])
...
def init_params(options):
    ...
    params['embeding_W'] = norm_weight(options['n_words'], options['dim_word'])
...
# build a sampler
def build_sampler(tparams, options, trng,use_noise=None, sampling=True):
   ...
   f_init = theano.function([ctx,K.learning_phase()], [ctx]+init_state+init_memory, name='f_init', on_unused_input='warn',profile=False)
   print 'Done'
   ...
   emb = tensor.switch(x[:,None] < 0, tensor.alloc(0., 1, tparams['embeding_W'].shape[1]),
                       tparams['embeding_W'][x])
   ...
   f_next = theano.function([x, ctx, K.learning_phase()]+init_state+init_memory, [next_probs, next_sample]+next_state+next_memory, 
                             on_unused_input='warn', name='f_next', profile=False)

   return f_init, f_next

#from Core.engine import build_sampler
# generate sample
def gen_sample(tparams, f_init, f_next, ctx0, options,
               trng=None, k=1, maxlen=30, stochastic=False, train=0):
    ...
    for ii in xrange(maxlen):
        ...
        rval = f_next(*([next_w, ctx0,train]+next_state+next_memory))
        next_p = rval[0]
        next_w = rval[1]
...
def train(dim_word=100,  # word vector dimensionality
    ...
    #inps += [K.learning_phase()] 
    f_log_probs = theano.function(inps, -cost, profile=False, on_unused_input='warn',
