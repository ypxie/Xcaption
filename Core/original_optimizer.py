...
def adadelta(lr, tparams, grads, inp, cost, hard_attn_up=[]):
    ...
    f_update = T.function([lr], [], updates=ru2up+param_up, on_unused_input='ignore', profile=False)
...
def rmsprop(lr, tparams, grads, inp, cost, hard_attn_up=[]):
    zipped_grads = [T.shared(p.get_value() * numpy.float32(0.), name='%s_grad'%k) for k, p in tparams.iteritems()]
    running_grads = [T.shared(p.get_value() * numpy.float32(0.), name='%s_rgrad'%k) for k, p in tparams.iteritems()]
    running_grads2 = [T.shared(p.get_value() * numpy.float32(0.), name='%s_rgrad2'%k) for k, p in tparams.iteritems()]
    ...
    f_update = T.function([lr], [], updates=updir_new+param_up, on_unused_input='ignore', profile=False)
...
def adam(lr, tparams, grads, inp, cost, hard_attn_up=[]):
    ...
    f_update = T.function([lr], [], updates=updates, on_unused_input='ignore')
