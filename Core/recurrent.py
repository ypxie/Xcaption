...
def init_dynamic_lstm_cond(options, params, prefix='lstm_cond', nin=None, dim=None,
    ...
    for idx, ctx_dim in enumerate(ctx_dim_list):
        ...
        Wc2h = init((proj_ctx_dim, dim * 4), symbolic=False)
...
def dynamic_lstm_cond_layer(tparams, state_below, options, prefix='dlstm', mask=None, context_list=[None],
    ...
    def _step(m_, x_, h_, c_, wr_tm1, wr_tm2, wr_tm3, pctx_1, pctx_2, pctx_3):
        ...
        # pstate_collect = [None for _ in range(num_attention)]
        # for idx, pctx_ in enumerate(pctx_collect):
        #     if options['project_context']:
        #         pstate_ = T.dot(h_, tparams[get_name(prefix,'W_h2pc_cidx_%d'%idx)])
        #         pctx_ = pctx_ + pstate_[:,None,:]
        #         # the most tricky line of code, just to provent the pcts does not have sample dimension
        #         #pcts_ can be (nsample, 196,512) or can be (196, 512), h_ can also be n*dim or can be 1*dim
        ...
        #
        #     pctx_collect[idx] = pctx_
        #     pstate_collect[idx] = pstate_
        #
        ...
        alpha_sample_list = []
        att_ctx_list = []
        for proj_context, alpha in zip(pctx_collect, alpha_list):
            ...
            if options['hard_sampling'] == True or options['attn_type'] == 'stochastic':
                if sampling:
                    ...
                    if argmax:
                        ...
                        alpha_sample = alpha
                att_ctx = (proj_context * alpha_sample[:, :, None]).sum(axis=1)  # current context
            else:
                att_ctx = (proj_context * alpha[:, :, None]).sum(axis=1)  # current context
                alpha_sample = alpha  # you can return something else reasonable here to debug
            alpha_sample_list.append(alpha_sample)

            if options['project_context']:
                att_ctx._keras_shape = (None, None, options['proj_ctx_dim'])
            else:
                att_ctx._keras_shape = (None, proj_context._keras_shape[-1])
    ...
    for idx, projctx in enumerate(pctx_collection):
        if hasattr(projctx, '_keras_shape'):
            ctx_keras_shape = projctx._keras_shape
